...
class CVVehicleWheels extends Migration
{
    ...
    public function up()
    {
         ...
         SELECT c_vehicle_wheels.id,
           ...
           FROM ((backendcommon.c_vehicle_wheels c_vehicle_wheels
                  LEFT OUTER JOIN backendcommon.c_wheels c_wheels
                     ON (c_vehicle_wheels.wheel_id = c_wheels.id))
                 LEFT OUTER JOIN backendcommon.w_products w_products
                    ON (c_vehicle_wheels.product_id = w_products.id))
                LEFT OUTER JOIN backendcommon.w_ware_houses w_ware_houses
